...
def get_menu(hall: str, food_type: str, db: Annotated[Session, Depends(get_db)]):
    ...
    appearance = db.execute(select(Appearance).where(Appearance.hall_id==hallID.id).where(Appearance.date == date).where(Appearance.food_type == food_type)).scalars().all()
    return appearance